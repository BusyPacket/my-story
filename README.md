...
| `description`     | 描述（可选）                                                                                         |
| `usage`           | 使用频率（可选），取值见下方「使用频率取值」，不填表示未统计                                         |
| `purchase_date`   | 购入日期                                                                                             |
| `purchase_method` | 购入方式（可选）：`taobao` / `jd` / `xianyu` / `custom` / `gift` / `other`，不填默认 `other`（其他） |
...
| `status`          | 当前状态（可选），取值见下方「状态取值」，不填默认持有中                                             |
| `status_date`     | 状态开始日期（可选），如已出二手 / 已丢弃 / 淘汰的日期，日均费用截止到该日                           |
...
| `daily`   | 每天使用 | 几乎每天都在用   |
| `weekly`  | 每周使用 | 每周用几次       |
| `monthly` | 每月使用 | 偶尔用，按月一次 |
| `rarely`  | 很少使用 | 基本吃灰         |

同样存英文值，中文文案在卡片组件内映射显示；不填则卡片上不出现该标签。
与日均费用结合起来看，就能找出「买得贵但用得少」的东西。
